...
These candidate-gated notes describe the intended `v0.1.0` release contents before the public GitHub Release exists.
Artifact, package-channel, and VM compatibility claims remain tied to the release workflow, signed checksums, final
proof workflow, and support-matrix evidence.
...
| AppImage, deb, rpm | Built and published by the tagged release workflow. |
...
- `packaging/release-signing-public.pem` contains the project release public key, and the live repository must have the
  matching `LOOPWIRE_RELEASE_PRIVATE_KEY` secret.
- Bunny.net docs deployment requires `BUNNY_STORAGE_ZONE` and `BUNNY_ACCESS_KEY` repository secrets.
