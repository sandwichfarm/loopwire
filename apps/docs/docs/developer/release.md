...
Custom local path inputs for `release:status`, including `--docs-deployment-manifest`, `--docs-dist`,
`--vm-evidence-root`, and `--support-matrix`, reject traversal, root/home-expanded paths, URL syntax, glob
metacharacters, symlinks, and existing paths with the wrong file or directory type before the audit begins.
