...
to Final Release Proof, including the GitHub Release asset list and signed archive downloads, `--vm-start-port 2600`
to align VM evidence collection handoffs with the rendered SSH plan, or `--skip-gh` when you only want local evidence
checks. Pinned Deploy Docs run ids still have to expose both `loopwire-docs` and
...
matched and missing ports for each requirement. Add `--include-jack-provider-plan` for provider-backed JACK reports; the
bundle writes `jack-provider-plan.json` and summarizes deterministic Loopwire-owned requirements as `jackProvider`
without running a JACK provider. Add `--include-dsp-provider-plan` for DSP restore or per-edge gain triage; the bundle
then writes `dsp-provider-plan.json` and summarizes read-only DSP provider operations as `dspProvider` without running
provider execute mode. It never uploads data automatically.
...
Final Release Proof dispatch. When `release:handoff --vm-evidence-asset NAME` is used, the rendered
`pnpm vm:prepare-release-evidence` command receives the same `--asset-name NAME` that upload, final proof, and status
audit commands expect. `--env-file` consumes only
...
The release-key prompt asks for a local PEM private-key path, validates it against the public key, and sends the file
contents as `LOOPWIRE_RELEASE_PRIVATE_KEY`.
