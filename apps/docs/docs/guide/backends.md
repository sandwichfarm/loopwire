...
selection; if none are available, it stays in a blocked diagnostics state. Both automatic single-backend selection and
manual backend changes run as a `backend-change` runtime transaction: Loopwire applies and verifies the active
