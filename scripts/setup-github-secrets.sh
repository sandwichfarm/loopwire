...
prompt_release_public_key() {
  ...
  printf '\nLOOPWIRE_RELEASE_PUBLIC_KEY_FILE\n' >&2
  printf '%s\n' "Public key used only to validate LOOPWIRE_RELEASE_PRIVATE_KEY before any secret is written. Enter a local public-key PEM path, paste the full public PEM block from BEGIN PUBLIC KEY through END PUBLIC KEY, or press Enter to use: ${default_value}" >&2
  ...
  while :; do
    ...
    release_public_key_file="$answer"
    return
  done
}
