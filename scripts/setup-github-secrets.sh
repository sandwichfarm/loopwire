...
Interactive set mode:
  ...
  prompt asks for a local PEM file path; the release workflow uses this key to
  sign SHA256SUMS so installers can verify published artifacts, then the file
  contents are sent as LOOPWIRE_RELEASE_PRIVATE_KEY.
...
prompt_missing_inputs_for_set_scope() {
  ...
  if [ "$scope" = "final" ]; then
    ...
    prompt_line \
      ...
      false \
      "" \
      "Final-release signing key, not a Bunny.net or GitHub token. Provide the path to the local PEM private key generated by pnpm release:prepare-key. The release workflow uses it to sign SHA256SUMS so installers can verify published artifacts; the helper validates the key pair, then sends the private-key file contents directly to the LOOPWIRE_RELEASE_PRIVATE_KEY GitHub secret."
