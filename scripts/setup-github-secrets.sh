...
Interactive set mode:
  ...
  prompt accepts a local PEM file path or a pasted PEM block; the release workflow
  uses this key to sign SHA256SUMS so installers can verify published artifacts,
  then the file contents are sent as LOOPWIRE_RELEASE_PRIVATE_KEY.
...
read_private_key_prompt_line() {
  ...
  if [ -t 0 ]; then
    if ! IFS= read -r -s answer; then
      printf '\n' >&2
      fail "missing input for LOOPWIRE_RELEASE_PRIVATE_KEY"
    fi
    printf '\n' >&2
  else
    if ! IFS= read -r answer; then
...
prompt_release_private_key() {
  ...
  while :; do
    ...
    if [ -n "$answer" ]; then
      release_private_key_file="$answer"
      return
    fi
