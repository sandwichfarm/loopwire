...
Latest release-status hardening: 2026-07-06 - `scripts/audit-final-release-state.sh` now accepts validated
`--release-evidence-asset` and `--vm-evidence-asset` overrides. `scripts/plan-final-release-handoff.sh` forwards the
same archive names from final-proof dispatch into the closing `pnpm release:status` audit, and `pnpm verify:scripts`
proves the fake GitHub download path uses those override names.
...
- Full audio graph mutation is intentionally incomplete. The PulseAudio compatibility adapter covers Loopwire null
  ...
  mixing semantics. Desktop Host apply can now run a saved live DSP provider after checking `capabilities` for
...
- Live JACK client creation, bundled live backend DSP capture/injection, native graph-edge gain, and published release
  proof remain the major product gaps for a fully functional Loopback-class app.
...
- 2026-07-06 release status evidence asset override hardening: `scripts/audit-final-release-state.sh` can now audit
  ...
  dispatch into its closing `pnpm release:status` command. `scripts/verify-scripts.sh` checks help text, unsafe-name
  rejection, handoff propagation, and fake GitHub download patterns for both override assets. Focused validation
  passed: `bash -n scripts/audit-final-release-state.sh scripts/plan-final-release-handoff.sh
...
- 2026-07-06 agent-ready evidence asset handoff hardening: `scripts/verify-agent-release-ready.sh` now asserts that the
  rendered final release handoff carries tag-bound release and VM evidence archive names through both final-proof
  workflow dispatch and the final `pnpm release:status` audit. `scripts/verify-scripts.sh` checks those command paths in
  the agent-ready smoke. Focused validation passed: `bash -n scripts/verify-agent-release-ready.sh
  scripts/verify-scripts.sh scripts/verify-docs.sh`, `pnpm verify:scripts`, `pnpm verify:docs`, and
...
- 2026-07-06 Desktop DSP live provider apply: desktop Host apply now unblocks DSP Provider only when saved provider
  ...
  configuration runtime adapter for graph-edge apply, verify, and rollback. The Tauri audio command bridge now carries
  provider stdin for rendered buffers and allowlists only the bounded DSP provider protocol shapes. Focused validation
  passed so far: `pnpm --filter @loopwire/desktop test -- live-apply-preflight route-control-semantics`,
  `pnpm --filter @loopwire/desktop typecheck`, `pnpm verify:tauri`, `pnpm verify:docs`, `pnpm verify:scripts`,
  `pnpm --filter @loopwire/desktop build`, `pnpm verify:desktop-preview -- --screenshot-dir
  /tmp/loopwire-dsp-live-provider-preview`, and `git diff --check`. Desktop preview wrote desktop/mobile screenshots
  and reported no horizontal overflow. No secret write, release tag, public release, Bunny deployment, final proof
...
- 2026-07-06 Pinned docs-run artifact proof: `pnpm release:status --docs-deployment-run-id` now verifies the pinned
  ...
  and avoids pinning the embedded handoff to that run. Live read-only probe against Deploy Docs run `28759926595`
  blocked as expected because the run only exposes `loopwire-docs`. Focused validation passed: `bash -n
  ...
  packaging/release-signing-public.pem --skip-gh --skip-tag --skip-clean-git --allow-candidate-notes`; live explicit
  artifact probe with `pnpm release:status -- --repo sandwichfarm/loopwire --tag v0.1.0 --git-head
  e94b1fcea38f7db41017dd18d733e371861cb4b2 --docs-deployment-run-id 28759926595 --secret-list-file
  scripts/fixtures/github-secret-list-final.tsv`; and `git diff --check`. Full validation passed: `pnpm check`. No
...
- 2026-07-06 Docs deployment run selection proof: added `scripts/select-docs-deployment-run.sh` and the
  ...
  `pnpm release:fetch-docs-proof`, final proof dispatch, and `pnpm release:status`. The selector filters Deploy Docs
  runs by `--git-head`, requires completed successful status, and requires both `loopwire-docs` and
  `loopwire-docs-deployment` artifacts before printing the run id. Focused validation passed: `bash -n
  scripts/select-docs-deployment-run.sh scripts/plan-final-release-handoff.sh scripts/verify-agent-release-ready.sh
  scripts/verify-scripts.sh`; `pnpm verify:scripts`; `pnpm verify:docs`; `pnpm verify:release-readiness -- --repo
  sandwichfarm/loopwire --tag v0.1.0 --public-key packaging/release-signing-public.pem --skip-gh --skip-tag
  --skip-clean-git --allow-candidate-notes`; `pnpm release:agent-ready -- --repo sandwichfarm/loopwire --tag v0.1.0
  --git-head $(git rev-parse HEAD) --allow-dirty --skip-local-gates`; and `git diff --check`. Full validation passed:
  `pnpm check`. No secret write, release tag, public release, Bunny deployment, final proof dispatch, VM launch, host
...
- 2026-07-05 Tag-bound final proof workflow status: the manual Final Release Proof workflow now uses the visible run
  ...
  `bash -n scripts/audit-final-release-state.sh scripts/verify-scripts.sh scripts/verify-github-workflows.sh
  scripts/verify-docs.sh`; `pnpm verify:workflows`; `pnpm verify:docs`; `pnpm verify:scripts`; and
  `git diff --check`. Full validation passed: `pnpm check`. No secret write, release tag, public release, Bunny
  deployment, final proof dispatch, VM launch, host audio mutation, or support-matrix promotion was performed.
- 2026-07-05 Final release status audit handoff: the generated release ceremony now ends with `pnpm release:status`
  after final-proof dispatch and local final-proof dry-run, so operators finish publication on the same read-only
  aggregate audit that checks published release assets, docs deployment proof, final-proof workflow status, VM
  evidence, and support-matrix claims. Focused validation passed:
  `bash -n scripts/plan-final-release-handoff.sh scripts/verify-scripts.sh scripts/verify-docs.sh`;
  `pnpm verify:scripts`; `pnpm verify:docs`; rendered `pnpm release:handoff -- --repo sandwichfarm/loopwire --tag
...
- 2026-07-05 Exact-commit agent-ready release handoff: the generated final release handoff now starts with
  ...
  final proof. Focused validation passed: `bash -n scripts/plan-final-release-handoff.sh scripts/verify-scripts.sh
  scripts/verify-docs.sh`; `pnpm verify:scripts`; `pnpm verify:docs`; rendered
  `pnpm release:handoff -- --repo sandwichfarm/loopwire --tag v0.1.0 --git-head $(git rev-parse HEAD)` output; and
  `git diff --check`. Full validation passed: `pnpm check`. No secret write, release tag, public release, Bunny
  deployment, VM launch, host audio mutation, or support-matrix promotion was performed.
- 2026-07-05 Stale saved backend replacement prompt: the desktop backend choice callout now names a previously saved
  backend when it is no longer detected and multiple other backends are available, making the replacement action
  explicit before live apply or startup restore are saved again. Focused validation passed:
  `pnpm --filter @loopwire/desktop test -- backend-choice`; `pnpm --filter @loopwire/desktop typecheck`;
...
- 2026-07-05 Release status tag-ref proof: `scripts/audit-final-release-state.sh` now verifies the live GitHub release
  ...
  fixture in `scripts/verify-scripts.sh` now covers mismatched tag refs and annotated-tag success. Focused validation
  passed: `bash -n scripts/audit-final-release-state.sh scripts/verify-scripts.sh scripts/verify-docs.sh`;
  `pnpm verify:scripts`; `pnpm verify:docs`; and `pnpm release:status -- --repo sandwichfarm/loopwire --tag v0.1.0
...
- 2026-07-05 Release status release-evidence archive proof: `scripts/audit-final-release-state.sh` now downloads the
  ...
  release public key, successful published-release smoke, and absence of release blockers. The fake GitHub fixture in
  `scripts/verify-scripts.sh` now covers missing downloadable release evidence assets, release-evidence-only success
  before a missing VM archive, and a signed dual-archive success path. Focused validation passed:
  `bash -n scripts/audit-final-release-state.sh scripts/verify-scripts.sh scripts/verify-docs.sh`;
  `pnpm verify:docs`; `pnpm verify:scripts`; and `pnpm release:status -- --repo sandwichfarm/loopwire --tag v0.1.0
  --git-head $(git rev-parse HEAD) --skip-gh`. Full validation passed: `pnpm check`; `git diff --check`; added-line
  length scan; and codebase-memory MCP fast index refresh with 3417 nodes and 6678 edges.
- 2026-07-05 Atomic docs deployment proof fetch: `scripts/fetch-docs-deployment-proof.sh` now downloads docs and
  deployment-manifest artifacts into a temporary staging directory, verifies the staged manifest against the staged docs
  bytes and expected commit, then replaces the durable local proof paths only after verification passes. The fake GitHub
  fixture in `scripts/verify-scripts.sh` now follows the staged layout and asserts that a missing deployment artifact
  leaves no partial docs dist or manifest in the requested output paths. Focused validation passed:
  `bash -n scripts/fetch-docs-deployment-proof.sh scripts/verify-scripts.sh scripts/verify-docs.sh`;
  `pnpm verify:docs`; and `pnpm verify:scripts`. Full validation passed: `pnpm check`; `git diff --check`; added-line
  length scan; and codebase-memory MCP fast index refresh with 3416 nodes and 6678 edges.
...
- 2026-07-05 Release readiness local artifact path hardening: `scripts/verify-release-readiness.sh` now validates
  ...
  readiness failure summary, and provided secret-list artifacts must exist as regular files. Root/home placeholders,
  parent/current traversal, URL syntax, glob metacharacters, symlinks, and existing non-file paths fail closed.
  `scripts/verify-scripts.sh` now rejects symlinked public-key paths and directory-valued secret-list paths. Release
  docs, unreleased notes, and `scripts/verify-docs.sh` document and assert the release-readiness file boundary.
  Focused validation passed: `bash -n scripts/verify-release-readiness.sh scripts/verify-scripts.sh
  scripts/verify-docs.sh`; offline release readiness positive smoke; symlink public-key and directory secret-list
  negative probes with expected errors; `git diff --check`; `pnpm verify:scripts`; and `bash scripts/verify-docs.sh`.
  Full local validation passed: `pnpm check`.
...
- 2026-07-05 VM evidence asset release-dir hardening: `scripts/prepare-vm-evidence-release-asset.sh` now validates
  ...
  closed before the helper can package VM evidence, regenerate `SHA256SUMS`, sign `SHA256SUMS.sig`, or print upload
  commands. `scripts/verify-scripts.sh` now rejects parent-traversal and symlink release directories in dry-run mode.
  Release docs, unreleased notes, and `scripts/verify-docs.sh` document and assert the release-dir safety contract.
  Focused validation passed: `bash -n scripts/prepare-vm-evidence-release-asset.sh scripts/verify-scripts.sh
  scripts/verify-docs.sh`, `git diff --check`, `pnpm verify:scripts`, and `bash scripts/verify-docs.sh`. Full local
  validation passed: `pnpm check`.
